...
class ResponseInputTest < ActiveSupport::TestCase
  test "new_from_params should create a new ResponseInput object" do
    ...
    params = ActionController::Parameters.new(problem_id: problem.id, player_id: player.id, response: (problem.solution + 1).to_s, started_at: Time.now.to_i.to_s)
    ...
    assert_equal(player, response_input.player)
